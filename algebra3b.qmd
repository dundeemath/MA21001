...
::: {#exm-vktspace6}
Let $\mathcal{P}_{n}$ denote the set of all polynomials
...
\vdots &  &  & 
\end{array}
\right|$ are identical, so it is zero (\ref{calcdet} 4). ◻
:::
...
$\det(A) =a_{1,1}a_{2,2}\cdots a_{n,n}$, the product of its diagonal
entries.
:::
...
::: {.Lemma}
If $A$ is $n\times n$, we have$,$ by Corollary \ref{caldet}
$$\begin{aligned}
...
::: {.callout-note}
## Note: 
Various authors define non-singular matrices to be
- invertible matrices
