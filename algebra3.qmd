...
<!-- author: Gunnar Hornig
  \
  ![image](dundee-logo.png){width="4cm"}
date: |
  August 2024\
...
::: example
*Example 2.2*. The set of all polynomials with degree $\leq n$ is
...
and $a,b\in \mathbb{R}$ : 
 ...
 $$\mbox{there exists a vector } {\vec 0}  \in V \mbox{such that } \vec{u}+\vec{0} = \vec{0}+\vec{u}=\vec{u}$${#eq-cond4}
 $$ \mbox{given } \vec{v} \mbox{ there exists a vector } -\vec{v}  \in V  \mbox{ such that }\vec{v}+ (-\vec{v})  = \vec{0}$$ {#eq-cond5} 
...
@eq-cond0 and @eq-cond1 ensure that both operations do not lead to elements
...
Eq. @eq-cond6 states that the neutral element of the multiplication in $\mathbb{R}$ is also
...
::: example
*Example 2.6*. What we have shown for a vector space consisting of two
...
::: example
*Example 2.7*. The set of all $m\times n$ matrices where the entries are
...
and zeros everywhere else. The space is of dimension $m n$.
:::

::: example
*Example 2.8*. Let $\mathcal{P}_{n}$ denote the set of all polynomials
with degree $\leq n$. $\mathcal{P}_{n}$ is a vector space if we declare
an addition and scalar multiplication of polynomials
...
space consists of the *monomials* $x^{r}, r=0,1, ..,n$ and is $n+1$
dimensional.
:::

::: example
*Example 2.9*. The solutions of a homogeneous linear differential
...
This is an example of a so-called *function space*.
:::

::: example
*Example 2.10*. The set ${\cal F}$ of all infinitely differentiable
functions, $\mathbb{R} \rightarrow \mathbb{R}$, i.e., those that have
derivatives of all orders. Note that (among others) the following
functions belong to ${\cal F}$: $$e^{nx},\,\cos \left( 2\pi nx\right)
...
::: example
*Example 2.11*. The set \ref{vksoverk} $\mathbb{C}^{n} =\{ (c_{1},...,c_{n})^{T}| c_{1},..,c_{n}\in \mathbb{C}\}$
...
::: example
*Example 2.12*. Are the vectors $(1,-1,0)^{T},(2,1,1)^{T}$ and
...
::: example
*Example 2.13*. Are the vectors $(1,0,0)^{T}$, $(1,1,0)^{T}$ and
...
$a_{1} = a_{3}$. So there are non-trivial solutions $(\lambda , -\lambda , \lambda )$, e.g. $(1,-1,1)$.
:::

::: example
*Example 2.14*. A set of two vectors is linearly independent if they are
...
::: example
*Example 2.15*. Show that the vectors
...
::: example
*Example 2.19*. The set of vectors
$\left\{(1,0,0)^{T}, (0,1,0)^{T}, (0,0,1)^{T}\right\}$ forms a basis of
$\mathbb{R}^{3}$ (more generally $K^{3}$ for a field $K$), the set of
vectors $\left\{(1,0,0,0)^{T}, (0,1,0,0)^{T},\right\}$\
$\left (0,0,1,0)^{T},(0,0,0,1)^{T}\right\}$ forms a basis of
$\mathbb{R}^{4}$ (more generally $K^{4}$) and so on. This is called the
**standard basis** of $\mathbb{R}^{n}$ ($K^{n}$ in more general sense)
for $n\in\mathbb{N}$.
:::

::: example
*Example 2.20*. The vectors
...
::: example
*Example 2.22*. Consider the set of all $n\times n$ matrices with real
entries $\mathbb{R}^{(n\times n)}$. Let $E_{ij}$ be the $n\times n$
...
::: example
*Example 2.24* (Basis). A different basis, other than the standard
...
::: example
*Example 2.26*. The space of polynomials of degree $\leq n$ has basis
...
::: {.Corollary}
Every set of m+1 vectors in $\mathbb{R}^{m}$ is
linearly dependent.
:::

::: {.Proof}
We know that the standard basis of $\mathbb{R}^{m}$ consists of
m elements, hence the dimension is m. Suppose there were $m+1$ linearly
independent vectors in $\mathbb{R}^{m}$, then they could be used to form
a basis. But then the dimension of $\mathbb{R}^{m}$ would be $m+1$, in
contradiction to the previous result that the dimension is a unique
number. ◻
:::
...
::: example
*Example 2.27*. For any vector space $V$, $V$ is always a subspace of
itself.
:::

::: example
*Example 2.28*. We also always have a subspace $\{\vec{0}\}$ consisting
...
::: example
*Example 2.29*. The subspaces of $\mathbb{R}^{2}$ are $\{\vec{0}\}$,
lines through the origin, and $\mathbb{R}^{2}$ itself.
:::

::: example
*Example 2.30*. The subspaces of $\mathbb{R}^{3}$ are $\{\vec{0}\}$,
lines through the origin, planes through the origin, and
$\mathbb{R}^{3}$ itself.
:::

::: example
*Example 2.31*. Let $W=\{(x,y,z)^{T}:y=2x, z=0,x,y,z\in\mathbb{R}\}$.
This is the set of points lying on the line $y=2x$ in the plane $z=0$.
$W$ is a subspace of $\mathbb{R}^{3}$.
:::

::: example
*Example 2.32*. The set $W=\{(x,y,z,w)^{T}:x+y+z=1\}$ is not a subspace
...
::: example
*Example 2.33*. Consider the subset $S$ of $P_{2}$ defined so as to
...
::: example
*Example 2.34*. Let $V= \mathbb{R}^{2\times 2}$ and define $W_{1}$ to be
...
::: example
*Example 2.35*. Let $V=\mathbb{R}^{2}$, let
...
::: example
*Example 2.36*. The set $W=\{(x,y,z)^{T}:x^{2}+y^{2}=z\}$ is not a
...
::: example
*Example 2.37*. The set $W=\{(x,y,z,w)^{T}:x+y+z=w\}$ is a subspace of
...
Now $col(A) \neq col(R)$, but the columns of $A$ and $R$ have the same
dependence relationships. Therefore, $dim(col(A)) = dim(col(R))$. Since
there are $\gamma$ pivots, $R$ has $\gamma$ columns that are linearly independent, and the remaining columns
of $R$ are linear combinations of them. Thus, $dim(col(R)) = \gamma$. It follows that $dim(row(A)) = \gamma = dim(col(A))$, as we wished to prove. ◻
:::

::: {.Definition}
## Rank and nullity
The rank of a matrix $A$ is the dimension of
its row and column spaces and is denoted by ${\rm rank}(A)$. The nullity of a matrix $A$ is the dimension
of its nullspace and is denoted by ${\rm nullity}(A)$.
:::
...
Since $dim({\rm Null}(A)) = n - \gamma$, we have ${\rm rank}(A) + {\rm nullity}(A) = \gamma + (n- \gamma ) = n$. 
:::
...
We now apply row operations to reduce $N$ to a row echelon form given by
...
We see that there are only two nonzero rows, so ${\rm rank}(N)=2$. Hence,
...
That the matrix multiplication is not commutative in
general is clear already for dimensional reasons. If
...
$(m=r=n)$, the product is not commutative as the following example
shows: 
$$\begin{pmatrix}
 0 & 0\\
 1 &0
...
\end{pmatrix} ,\quad C= \begin{pmatrix}
 ...
 2  \\
 1  \\
\end{pmatrix} .$$ *AB*, *CA*, *CB* are not defined.
$$BA= \begin{pmatrix}
 3  & 1 & 1 \\
 6  & 4 & 3 \\
...
::: defn
**Definition 3.4** (Transpose of a Matrix). If $A=[a_{i\,j} ]$ is an
$m\times n$ matrix, then the transpose of $A$, denoted by $A^T$, is
defined as follows: $$A^T=[a_{j,i}]  \in M_{n\times m}  .$$
:::
...
::: proof
(i) follows directly from the definition. (ii):
...
::: defn
**Definition 3.5**. If ${\rm A}={\rm A}^T$ then ${\rm A}$ is said to be
symmetric, while if ${\rm A}= - {\rm A}^T$ then ${\rm A}$ is
skew-symmetric. Note that this implies that ${\rm A}$ is a square
matrix.
:::
...
::: defn
**Definition 3.6**. Given two matrices ${\rm A}$ and ${\rm B}$ which
satisfy
...
::: prop
**Proposition 3.1**. *If ${\rm A}$ and ${\rm B}$ are non-singular
...
called *Gauss elimination* to determine solutions.
...
\end{aligned}$$ The result $y=-3$ is then substituted back in either of
...
That is, we converted the system $$\begin{aligned}
2x + 3y  &= & 1 ,\\
2x+ 2y &= 4 \ .
\end{aligned}$$ into the equivalent system $$\begin{aligned}
2x + 3y  &= & 1 ,\\
      - y &= 3 \ , 
\end{aligned}$$ by subtracting equations. \"Equivalent\" here means that
...
write the system as a matrix, called *augmented matrix*. For the example
...
::: definition
*Definition 3.1*. An elementary matrix is an $n\times n$ matrix which
...
::: example
*Example 3.4*. $R_{1}\leftrightarrow R_{3}$:
...
::: example
*Example 3.5*. $3R_{1}\rightarrow R_{1}$:
...
::: example
*Example 3.6*. $R_{2}-R_{1}\rightarrow R_{2}$:
...
::: thm
**Theorem 3.3**. *The elementary matrices are nonsingular. Furthermore,
their inverse is also an elementary matrix. That is, we have:*

- *The inverse of the elementary matrix which interchanges two rows is
  itself. For example the inverse of
  $\left(\begin{array}{ccc}0 & 1 & 0\\ 1 & 0 & 0 \\ 0 & 0 & 1\end{array}\right)$
  is itself
  $\left(\begin{array}{ccc}0 & 1 & 0\\ 1 & 0 & 0 \\ 0 & 0 & 1\end{array}\right)$.*
...
- *The inverse of the elementary matrix which multiply a row $i$ by a
  ...
  which multiply a row $i$ by $\frac{1}{a}$, i.e.
...
[\[matrixinv1\]](#matrixinv1){reference-type="ref"
reference="matrixinv1"} it follows by multiplying successively with the
...
::: definition
*Definition 3.2*. A square matrix is *upper triangular* if all its
entries below the main diagonal are zero.

A square matrix is *lower triangular* if all its entries above the main
diagonal are zero.
:::
...
\end{aligned}$$ where $\lambda =d/a$, $\mu =g/a$, $\omega =h'/e'$. Note that this can be accomplished by multiplying
${\rm A}$ from the left by the elementary matrices $${\rm E}_1 =\left( 
{{\begin{array}{rrr}
...
::: example
*Example 3.8*. Let $${\rm A} =\left(\begin{array}{rrr}
 1  & 2  & 1  \\
 2  & {-1}  & 3  \\
 {-1}  & 2  & 2  
...
Determinants occur naturally as ($\pm$) the volume of the parallelepiped
spanned by a set of $n$ vectors. In two dimensions (x-y-plane) the area
(2-dimensional volume) of a parallelepiped spanned by the two vectors
${\bf b}=(b_{1},b_{2})^{T}$ and ${\bf c}=(c_{1},c_{2})^{t}$ is up to a
sign given by $$area = \pm(b_{1}c_{2}- b_{2} c_{1} )$$ We call this the
determinant of the matrix ${\rm A} = [{\bf b},{\bf c}]$ and write
$${\rm det(A)}= |{\rm A}|=\left| 
\begin{array}{cc}
b_{1} & c_{1} \\ 
b_{2} & c_{2}
...
::: defn
**Definition 4.1** (Minor and Cofactor). Corresponding to each entry
$a_{i,j}$ in an $n\times n$ matrix $A,$ we define a number $M_{ij},$
...
::: {#detdefn .defn}
**Definition 4.2** (Determinants). A determinant of an $n \times n$
matrix can be expanded in terms of $(n-1)\times(n-1)$ determinants using
...
**Remember**: The rules are similar to, **but not the same as,**
...
determinant by a factor of $k$ (Corollary 2 tells us this). A further
...
about the 1st row $:$ $$\begin{tabular}[t]{ll}
...
::: cor
**Corollary 4.2**. *If $A$ is either lower or upper triangular, then
$\left|
...
::: example
*Example 4.9*. $\left|\begin{array}{rrr}
1 & 7 & -2 \\
0 & 14 & 32 \\
0 & 0 & -2
...
::: thm
**Theorem 4.3** (Product Rule). *For any two matrices $A$ and $B,$
$\det (AB)$ = $\det (A) \det (B)$.*
:::
...
::: cor
**Corollary 4.4**. *If $A$ is $n\times n$, we have$,$ by Theorem
[4.1](#calcdet){reference-type="ref" reference="calcdet"},*
...
::: cor
**Corollary 4.5**. *When $A$ is non-singular (i.e., invertible)
...
::: example
*Example 4.10*. Let $A$ be an $n\times n$ matrix with
...
::: cor
**Corollary 4.6**. *The matrix $A$ has an inverse if, and only if,
$\left|
A\right| \neq 0.$*
:::
...
::: cor
**Corollary 4.7**. *If $A$ is a square matrix, the system
...
::: defn
**Definition 4.3**. The transpose of the matrix of cofactors of $A$ is
called the *adjugate* or *adjoint* of ${\rm A}$ and denoted by
$\rm{adj}({\rm A}) =(\mbox{Cofactors})^{T}$.
:::
...
::: example
*Example 4.12*. Use the adjoint matrix to find the inverse of the matrix
...
::: example
*Example 4.13*. Find the inverse of $$A=\left(\begin{array}{rrr}
2 & 3 & 4 \\
5 & 6 & 7 \\
8 & 9 & 1
\end{array}\right).$$
...
::: example
*Example 4.14*. In the** **$2\times 2$ case, if $$\begin{aligned}
A&=\left[ 
\begin{array}{cc}
a & b \\ 
c & d
...
<figure id="eigenv1">
<p><img src="eigenv1.png" style="width:4cm" alt="image" /> <img
src="eigenv2.png" style="width:4cm" alt="image" /></p>
<figcaption>Effect of a deformation on the points (vectors) in an
elastic sphere</figcaption>
</figure>
...
<figure id="eigenv1">
<p><img src="eigenv3.png" style="width:4cm" alt="image" /> <img
src="eigenv4.png" style="width:4cm" alt="image" /></p>
<figcaption>There are directions in which the vectors only change their
length</figcaption>
</figure>
...
::: defn
**Definition 5.1**. Let $A$ be an $n \times n$ matrix. A scalar $\lambda$ is called an ***eigenvalue*** of $A$ if there is a nonzero vector
...
:::: example
*Example 5.2*. Show that $5$ is an eigenvalue of
...
::: defn
**Definition 5.2**. Let $A$ be an $n \times n$ matrix and let $\lambda$ be an eigenvalue of $A$. The collection of all eigenvectors
...
::: defn
**Definition 5.3**. For an $n \times n$ matrix $A$, the equation $|A - \lambda I_{n}| = 0$ is called the ***characteristic equation*** of $A$,
and $|A - \lambda I_{n}|$ is called the ***characteristic polynomial*** of $A$.
:::

::::::: example
*Example 5.3*. Find the eigenvalues and the corresponding eigenvectors
...
:::::::: example
*Example 5.4*. Find the eigenvalues and the corresponding eigenvectors
of
...
::: defn
**Definition 5.4**. The ***algebraic multiplicity*** of an eigenvalue is
...
::: thm
**Theorem 5.2**. *The eigenvalues of a triangular matrix are the entries
...
::: thm
**Theorem 5.3**. *Let $A$ be an $n \times n$ matrix and let $\lambda _{1}, \lambda _{2}, ..., \lambda _{m}$ be distinct eigenvalues of $A$ with corresponding
...
::: defn
**Definition 5.5**. Let $A$ and $B$ be $n \times n$ matrices. We say
...
::: rem
*Remark 5.1*. If $A \sim B$, we can write, equivalently, that
$A = PBP^{-1}$ or $AP = PB$.\
The matrix $P$ depends on $A$ and $B$. It is not unique for a given pair
of similar matrices $A$ and $B$.
:::

::: thm
**Theorem 5.4**. *Let $A$ and $B$ be $n \times n$ matrices with
$A \sim B$. Then\
(a) $det(A) = det(B)$\
...
$B = \left[\begin{array}{cc} 2 & 1 \\ 1 & 2 \end{array}\right]$ are not
similar, since $det(A) = -3$ but $det(B) = 3$.\
(b) $A = \left[\begin{array}{cc} 1 & 3 \\ 2 & 2 \end{array}\right]$ and
$B = \left[\begin{array}{cc} 1 & 1 \\ 3 & -1 \end{array}\right]$ are not
...
::: defn
**Definition 5.6**. An $n \times n$ matrix $A$ is ***diagonalisable***
if there is a diagonal matrix $D$ such that $A$ is similar to $D$ - that
is, if there is an invertible $n \times n$ matrix $P$ such that
$P^{-1}AP = D$.
:::

::: thm
**Theorem 5.5**. *Let $A$ be an $n \times n$ matrix. Then $A$ is
...
::::: rem
*Remark 5.2*. Eigenvectors can be placed into the columns of $P$ in any
...
::: thm
**Theorem 5.6**. *Let $A$ be an $n \times n$ matrix and let $\lambda _{1}, \lambda _{2}, \cdots, \lambda _{k}$ be distinct eigenvalues of $A$. If $B_{i}$ is a basis for
...
::: thm
**Theorem 5.7**. *If $A$ is an $n \times n$ matrix with $n$ distinct
...
theorem 4.3, $\mathbf{v_{1}}, \mathbf{v_{2}}, \cdots, \mathbf{v_{n}}$
are linearly independent, so, by Theorem 4.6, $A$ is diagonalisable. ◻
:::
...
has eigenvalues $\lambda _{1} = 2, \lambda _{2} = 1, \lambda _{3} = 3$ and $\lambda _{4} = -2$, by Theorem 4.2. Since these are four distinct
eigenvalues for a $4$x$4$ matrix, $A$ is diagonalisable, by Theorem 4.8.
::::

::: lem
**Lemma 5.8**. *If $A$ is an $n \times n$ matrix, then the geometric
...
distinct eigenvalues $\lambda _{1} = \lambda _{2} = 1$ and $\lambda _{3} = 2$. Since the eigenvalue $\lambda _{1} = \lambda _{2} = 1$ with
$E_{1} = {\rm span}\left(\left[\begin{array}{c} 1 \\ 1 \\ 1 \end{array}\right]\right)$
has algebraic multiplicity $2$ but geometric multiplicity $1$, $A$ is
...
two distinct eigenvalues $\lambda _{1} = 2$ and $\lambda _{2} = \lambda _{3} = 1$. We found:\
for $\lambda _{1} = 2$, $E_{2}$ has basis
$\mathbf{p_{1}} = \left[\begin{array}{c} 1 \\ 0 \\ -4 \end{array}\right]$\
for $\lambda _{2} = \lambda _{3} = 1$, $E_{1}$ has basis
$\mathbf{p_{2}} = \left[\begin{array}{c} -2 \\ 1 \\ 0 \end{array}\right]$
...
[^2] []{#Gauss label="Gauss"} Carl Friedrich Gauss (Gauß) (30 April
