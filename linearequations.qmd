...
$${\rm A} + {\rm B}=[a_{ij}] + [b_{ij}]= [ a_{ij}+b_{ij}], $$
and the scalar multiplication by
$$c {\rm A} = c [a_{ij}] = [ c a_{ij}] .$$
:::
...
defined as $$A^T=[a_{ji}]  \in M_{n\times m}  .$$
:::
...
symmetric. If ${\rm A}= - {\rm A}^T$ then ${\rm A}$ is said to be skew-symmetric or anti-symmetric. 
:::
...
$${\rm A} = \frac{{\rm A}^{T}+{\rm A}}{2} + \frac{{\rm A}- {\rm A}^{T}}{2} ,$$ {#eq-symdecomp}
...
\end{array}  \right) = {\rm A^T}; \quad  {\rm A} = \left(\begin{array}{rrr}
 ...
 -1  & 0  & 5  \\
 4  & -5  & 0  
\end{array}  \right) = (- {\rm A}^T).$$
Note that a skew symmetric matrix has to have zeros on the diagonal.  
:::


::: {#exm-} 
Example for the decomposition @eq-symdecomp.
$$\left( \begin{array}{rrr}
 2  & 0  & {-4}  \\
 {-2}  & 2  & 0  \\
 2  & 4  & 6  
...
There exists a neutral element, ${\rm I} \in M_{n\times n}$, w.r.t.~the matrix multiplication, called the
...
then ${\rm B} = {\rm A}^{-1}$ is called the inverse of ${\rm A}$ (and vice versa ${\rm A}$ is the inverse of ${\rm B}$). If
...
The definition above requires ${\rm A}{\rm B} = {\rm B}  {\rm A}$, which implies that ${\rm A}$ and ${\rm B}$ are square matrices of the same size. 
Hence only square matrices can have an inverse (see also remark -@nte-nonsquare).
:::

::: {.Proof}
Assume ${\rm A} \in M_{m\times r}$ and ${\rm B} \in M_{s\times n}$, then for ${\rm A}{\rm B}$ to be defined we have to have $r=s$ and for ${\rm B A}$ to be defined $m=n$. Furthermore ${\rm A B}= {\rm B A}$ requires $m=n$. 
:::

::: {.callout-note}
## Remark
Note that we have to require both ${\rm A} {\rm B} ={\rm I}$ and ${\rm B} {\rm A}={\rm A} {\rm B}$ since the matrix product is not commutative. Below is an example that ${\rm A} {\rm B} ={\rm I}$ alone is not sufficient to guarantee that ${\rm B}$ is the inverse of ${\rm A}$. Alternatively we can demand that both matrices are square matrices, ${\rm A},{\rm B}\in M_{n\times n}$, then ${\rm A} {\rm B} ={\rm I}$ is enough to show that ${\rm B}$ is the inverse of ${\rm A}$. The proof uses the fact that square matrices have determinants and  ${\rm A} {\rm B} ={\rm I}$ implies that the determinants of both matrices are non-zero. Hence both matrices have inverses and multiplying ${\rm A} {\rm B} ={\rm I}$ from the left with ${\rm A}^{-1}$ yields ${\rm B} = {\rm A}^{-1}$.
:::

::: {#exm-} 
Check that ${\rm A}{\rm B}= {\rm I}$ and ${\rm B}{\rm A} = {\rm I}$ and thus ${\rm B} = {\rm A}^{-1}$. 
...
::: {#exm-} 
Check that ${\rm A}{\rm B}= {\rm I}$, but ${\rm B}{\rm A} \neq {\rm A}{\rm B}$ and thus ${\rm B}$ is not the inverse of ${\rm A}$. 
$${\rm A}=\left[ \begin{array}{ccc}
 1 & 0 & 0 \\
 0 & 1 & 0 
\end{array}  \right], \quad 
...
In the previous sections, we encountered systems of equations of the type
$$a_{1} x_{1} + a_{2} x_{2}  + ... + a_{n} x_{n} = d.$$ In the
following, we will use a method
...
2x + 3y  & = 1.
\end{aligned} \right.$$ 
We use the curly bracket to remind us that this is a system, and that we need to solve both equations simultaneously. 
Usually we would solve the system by solving e.g. the first equation for $x$ and substituting the result in the second
equation: 
...
The result $y=-3$ is then substituted back in the other equation, and we find $x=5$. When it comes to systems with
...
Note that the second line above can be understood as the result of subtracting twice
the first equation from the second equation in the original system. 
...
x + y  &=  2 \\
      y &= -3 \  
\end{aligned} \right. $$
into an equivalent system by subtracting equations. \"Equivalent\" here means that
...
::: {#nte-effectivegauss .callout-note }
To avoid making unnecessary steps when applying row operations it is good to follow a scheme.
First, if your first row starts with a zero swap it for another row that starts with a non-zero number (ideally a small integer).
Second, use row operations to obtain zeros in the positions indicated below, starting in column one, row 2, and working your way to the end of the column before moving to column 2, row 3, and so on, as indicated in this matrix. Each time you subtract or add a multiple of row with the red element in the column to obtain zeros. 
$$\begin{array}{llll|r}
...
\end{array}$$ 
If in this process one of the red element is already zero, swap the row with a row below so that you get a non-zero element in the position of the red stars. This method avoids that you destroy zeros you already achieved by subsequent row operations. Below an example which does **not** follow this scheme and destroys existing zeros. 
$$\begin{array}{rrr|r}
  1 & 2& 1 &1 \\
   2& 1 & 1  & 2 \\
    1 & 3 & 3 & -3    
...
\begin{array}{rrr|r}
  1 & 2& 1 &1 \\
   0& -3 & -1  & 0 \\
    1 & 3 & 3 & -3    
\end{array}$$
$$\stackrel{R_3 + R_2}{\Longrightarrow}
\begin{array}{rrr|r}
  1 & 2& 1 &1 \\
   0& -3 & -1  & 0 \\
     1 & 0 & 2 & -3 
...
\begin{array}{rrr|r}
  1 & 0& 1 &1 \\
   0& -3 & -1  & 0 \\
     0 & -2 & 1 & -4 
\end{array}$$
In the last step we destroyed the zero in column , row 3, which we already had before, in order to get the one in column 1, row 3.
:::
...
\begin{array}{rrr|r}
  1 & 0& 1 &1 \\
   0& 1 & 1  & 2 \\
    0 & 0 & 0 & -8    
\end{array}$$ In this case, the last equation states: $0\, x + 0\, y + 0\, z=-8$,
which is impossible to satisfy for any $x,y$ or $z$. This case often occurs if
there are more equations than variables.
:::

::: {.callout-note collapse="true"}
## Additional material: Geometric interpretation
...
2.  The set of normal vectors of the hyperplanes is linearly dependent.
    ...
    (a)  One (or more) equations are linear combinations of the others.
    ...
    (b)  The equations are linearly independent and the system has no
        solution.
...
<figcaption>The left figure is the situation in Example @exm-ge3. The three
...
are performing the row operation $R_{3}-2 R_{1}\rightarrow R_{3}$.
:::
...
The elementary matrices are nonsingular, i.e. they have an inverse. Furthermore,
...
identity matrix with elementary row operations. Assume that there exists a sequence of row operations, with corresponding elementary matrices ${\rm E}_i$, that converts a square matrix ${\rm A}$ into an identity matrix.
...
::: {.callout-note}
To avoid making unnecessary steps when applying row operations it is good to follow the scheme explained in @nte-effectivegauss and go on to insert zeros also above the diagonal following the sequence indicated below. 
$$\begin{array}{llll|r}
...
onto the space of all possible $\vec{y}$ vectors, which is a subset
of $\mathbb{R}^{m}$.
...
Note first that the range of ${\rm A}\in M_{m\times n}$, that is all the vectors in $\mathbb{R^m}$ that are an image of a vector in $\mathbb{R^n}$ can be written as 
$$ {\rm range(A)} = \left\{ \vec{y} \in \mathbb{R}^m | \vec{y}={\rm A} \vec{x}; \ \vec{x} \in  \mathbb{R}^n \right\}. $$
The vectors $\vec{x} \in \mathbb{R}^n$ can be represented w.r.t. the standard basis:
...
The vector ${\rm A}\vec{e}_i$ is the i-th column of ${\rm A}$, hence the image of $\vec{x}$ is a linear combination of the columns of ${\rm A}$. Thus 
...
1. $\vec{0} = {\rm A} (\vec{0}) \in {\rm range(A)}$
2. if $\vec{y}_1={\rm A} (\vec{x}_1), \vec{y}_2 = {\rm A} (\vec{x}_2) \in {\rm range(A)}$ then 
$$\vec{y}_1 + \vec{y}_2 = {\rm A} (\vec{x}_1)+{\rm A} (\vec{x}_2) = {\rm A} (\vec{x}_1 + \vec{x}_2) \in {\rm range(A)}$$
3. if $c\in \mathbb{R}$ and $\vec{y}={\rm A} (\vec{x})$ then $c \vec{y}= c {\rm A} (\vec{x}) = {\rm A} (c \vec{x}) \in {\rm range(A)}.$
...
However, the set of vectors (columns of ${\rm A}$) is not linearly independent. A test for linear independence,
...
::::: {#exm-    }
In order for  $\vec{y}=A \vec{x}$ to have a solution $\vec{x}$, $\vec{y}$ has to be in the range of ${\rm A}$:   $\vec{y} \in {\rm range}(A)$. Let
...
Under what general conditions on $\vec{y} = \left(y_1,y_2, y_3\right)^T$ is the system consistent?
...
However, the three columns of ${\rm A}$ are not linearly independent. Indeed 
...
Hence, for example, the first two columns alone span ${\rm range}(A)$  
...
So $\vec{y} = \left(1,2,1\right)^T$ is in the range, but $\vec{y} = \left(1,1,1\right)^T$ is not. Note that the first and the third columns of $A$ also span ${\rm range}(A)$.
...
is the set of vectors ${\bf x}$ such that $A\vec{x} = \vec{0}$. The nullspace is also called kernel and denoted by ${\rm Null}(A)$. 
...
(i) Since $A\vec{0_n} = \vec{0_m},  \vec{0_n} \in {\rm Null}(A)$.

(ii) Let $\vec{u}$ and $\vec{v}$ be in ${\rm Null}(A)$. Therefore
$A\vec{u} = \vec{0}$ and $A\vec{v} = \vec{0}$. It follows
that $A(\vec{u} + \vec{v}) = A\vec{u} + A\vec{v}  = \vec{0} + \vec{0} = \vec{0}$ Hence, $\vec{u} + \vec{v} \in {\rm Null}(A)$.

(iii) Finally, for any scalar $\lambda$,
$A(\lambda \vec{u}) = \lambda (A\vec{u}) = \lambda \vec{0} = \vec{0}$ and therefore $\lambda \vec{u} \in {\rm Null}(A)$. It follows that ${\rm Null}(A)$ is
...
$rank(A) = \gamma$. Then $R$ has $\gamma$ pivots, so there are $\gamma$ variables corresponding to the leading entries and $n-\gamma$ free variables in the solution to $A\vec{x} = \vec{0}$.
...
Solving the equation $A\vec{x} = \vec{0}$, we see that all
solutions are of the form
$\vec{x} = c \left[1, 1, 1, -1 \right]^{T}$, so that the dimension of
...
into $\vec{x}$, and write the results as a linear combination of $f$
